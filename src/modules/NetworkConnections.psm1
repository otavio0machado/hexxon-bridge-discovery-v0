...
function Get-ProcessConnections { param([int]$Pid)
 $result=@();$cmd=Get-Command Get-NetTCPConnection -ErrorAction SilentlyContinue
 if($cmd){try{$result=@(Get-NetTCPConnection -OwningProcess $Pid -ErrorAction Stop | ForEach-Object {[pscustomobject]@{localAddress=$_.LocalAddress;localPort=$_.LocalPort;remoteAddress=$_.RemoteAddress;remotePort=$_.RemotePort;state=$_.State.ToString();pid=$Pid;source='Get-NetTCPConnection'}})}catch{}}
 if(!$result){$result=@((& netstat -ano -p tcp 2>$null | ForEach-Object {ConvertFrom-NetstatLine $_}) | Where-Object {$null -ne $_ -and $_.pid -eq $Pid})}
